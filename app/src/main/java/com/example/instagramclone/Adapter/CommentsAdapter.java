...
public class CommentsAdapter extends RecyclerView.Adapter<com.example.instagramclone.Adapter.CommentsAdapter.ViewHolder>{
        ...
        public class ViewHolder extends RecyclerView.ViewHolder {
            ...
            public void bind(Comment comment, ViewHolder viewHolder) {


                tvUserName2.setText(comment.getUser().getUsername());
                tvComment.setText(comment.getComments());
                Glide.with(viewHolder.itemView.getContext()).load(comment.getUser().getParseFile(User.KEY_Profile_Image).getUrl()).transform(new CircleCrop()).into(ivProfile_Image2);
